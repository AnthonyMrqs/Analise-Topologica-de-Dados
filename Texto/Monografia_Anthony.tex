...
\begin{titlepage}
    ...
    \begin{center}
        ...
        \textbf{Análise Topológica de Dados Aplicada a Biologia Molecular}
    \end{center}
...
\begin{figure}[H]
    ...
    \caption{A esquerda, o mecanismo de transição conformacional da MBP visualizado através de diagramas de fitas. O modelo destaca a estrutura secundária da proteína, onde as molas coloridas indicam as hélices-$\alpha$ e os fios finos correspondem aos \textit{loops} de conexão. A variação espacial dessas estruturas entre as formas \textit{Open}, \textit{Semi-closed} e \textit{Closed} demonstra de forma clara o movimento de dobradiça (\textit{hinge-bending}) responsável pela captura do ligante na fenda central. A direita, a vista superior da MBP, mostrando o tamanho e a forma do sítio de ligação que é modulado pelo estado conformacional da proteína. A cor vermelha indica que os resíduos estão próximos ao centro de massa da proteína, onde o sítio de ligação está localizado. As estruturas de raios X são mostradas para o estado aberto não ligado (entrada PDB 1OMP) e para o estado fechado ligado (entrada PDB 3MBP). Fonte: Adaptado de Bucher et al.~\cite{bucher2011}.}  
\end{figure}
...
\ident Do ponto de vista matemático para a classificação estrutural, o resultado fundamental do modelo ANM repousa na inversa pseudogeneralizada da Matriz Hessiana (Ver apêndice \ref{apx:anm}). A partir dos modos de baixa frequência (geralmente os 20 primeiros modos não triviais, que ditam os movimentos globais da proteína \cite{atilgan2001, kovacev2016}), constrói-se a Matriz de Correlação Cruzada ($C$). 
...
\ident A matriz de distâncias dinâmicas $D$ constitui o ponto de partida matemático deste trabalho. Ela nos permite abandonar as coordenadas euclidianas estáticas $(x, y, z)$ da biologia estrutural clássica e passar a tratar os resíduos da proteína como um conjunto discreto munido de uma medida de dissimilaridade, sobre o qual definiremos complexos simpliciais e calcularemos grupos de homologia persistente nos próximos capítulos. É crucial notar que $D$ não se qualifica formalmente como uma métrica clássica ou mesmo como uma pseudométrica. Primeiramente, a identidade dos indiscerníveis não se verifica de forma estrita: dois resíduos distintos ($i \neq j$) podem apresentar flutuações perfeitamente correlacionadas ($|C_{ij}| = 1$), resultando em $D_{ij} = 0$. Adicionalmente, a transformação $D_{ij} = 1 - |C_{ij}|$ operada sobre a matriz de correlação cruzada não garante o cumprimento geral da desigualdade triangular ($D_{ik} \leq D_{ij} + D_{jk}$), o que classifica o objeto estritamente como uma medida de dissimilaridade simétrica não-negativa. Todavia, a formulação computacional de um complexo de Vietoris-Rips filtrado prescinde do cumprimento desses axiomas métricos estruturais. O algoritmo opera a construção topológica como uma filtração sobre um grafo ponderado completo, no qual o parâmetro de filtração $t$ atua como o limiar de adjacência para a inserção de arestas e de simplexos de ordem superior, dependendo unicamente do valor escalar da dissimilaridade local entre os nós.


\section{Fundamentos de Topologia Algébrica e Homologia Persistente} \label{sec:homologia_persistente}

\ident O estabelecimento de uma matriz de distâncias (ou dissimilaridades) a partir de um sistema dinâmico fornece um espaço métrico discreto. Para extrair a arquitetura de comunicação global da biomolécula a partir desta dissimilaridade, traduzimos este espaço num objeto combinatório contínuo. Nesta seção, fundamentamos o arcabouço matemático necessário para essa transição, introduzindo conceitos da Topologia Algébrica e da Homologia Persistente, usando como base o conteúdo de Tinarrage \cite{tinarrage2021}.
...
\ident Por convenção, ao falar de simplexos, usamos colchetes em vez de chaves. Por exemplo, o simplexo $\{0, 1\}$ será denotado por $[0, 1]$, não confundir com a notação de índices. 
...
\begin{example}\label{ex_3_1}
    ...
    \end{align*}
    É um complexo simplicial de dimensão 1 (1-simplexo), onde todos os vértices se ligam. Pode ser representado dessa forma:
...
\begin{definition}
    ...
    Munido da topologia de subespaço, $(|K|, \tau_{|K|})$ é um espaço topológico (Ver apêndice \ref{apx:topologia}), que denominamos realização topológica de $K$.
...
\begin{example}\label{exemplo_fronteira}
    ...
    \begin{align*}
        \partial_1\left([0, 1] + [1, 2] + [2, 0]\right) &= \partial_1[0, 1] + \partial_1[1, 2] + \partial_1[2, 0]\\
        &= [0] + [1] + [1] + [2] + [2] + [0]\\
        &= 0
    \end{align*}
...
\ident A aplicação linear de fronteira $\partial_n$ satisfaz a propriedade fundamental da topologia algébrica: 
\[
\partial_{n-1} \circ \partial_n = 0.
\]
Esta identidade categórica permite-nos definir subespaços cruciais para o estudo das formas.
...
\begin{example}
    ...
    \end{align*}
    Vemos que as cadeias $[0, 2] + [2, 3] + [0, 3]$ e $[0, 1] + [1, 2] + [2, 3] + [0, 3]$ são homólogas, isto é, as estrturas das cadeias se correspondem.
    De fato,
    \begin{align*}
...
\begin{figure}[H]
    ...
    \caption{Note que neste complexo temos $\beta_0 = 1 $ (1 componente conexa), $\beta_1 = 2$ (2 buracos de dimensão 1 ou 2 triângulos não preenchidos) e $\beta_2 = 1$ (1 buracos de dimensão 2 ou 1 tetraedro não preenchido/oco).}
\end{figure}
...
\begin{theorem}
    ...
    Suponha que X seja um subconjunto de $\mathbb{R}^n$. Considere uma cobertura $\mathcal{U} = \left\{U_i\right\}_{1\leq i \leq N}$ de $X$ tal que cada um dos $U_i$ são bolas fechadas e convexas. Então $\mathcal{N}(\mathcal{U})$ é homotopia equivalente a X. (Ver figura \ref{cobertura_nervo}).
\end{theorem}
\vspace{-0.6em}
Demonstração em Tinarrage \cite{tinarrage2021}.
...
\begin{proposition}
    Seja $X \subset \mathbb{R}^n$ um conjunto finito. Seja $X \subset \mathbb{R}^n$ um subconjunto finito. Para todo $t \geq 0$, temos
    \[
...
\begin{example}
    ...
    \begin{figure}[H]
        ...
        \end{subfigure}
        \hfill
        \caption{Na figura a direita, podemos observar espessamento de $X$ com $t=0.2$ e disso obtemos que $\beta_0 = 3$, ou seja, 3 componentes conexas e $\beta_1 = 0$.}
    \end{figure}

    Agora, calculamos as curvas de Betti de $X$.
...
\begin{definition}
    Uma filtração de um complexo simplicial $E$ é uma família de subcomplexos $\mathbb{X} = (X^t)_{t \in \mathbb{R}^+}$ de $E$ que formam uma cadeia encaixante, isto é, para quaisquer $s, t \in \mathbb{R}^+$, se $s \leq t$, então $X_s \subseteq X_t$.
\end{definition}

\ident Seja $K$ um complexo simplicial com $n$ simplexos. Escolha uma ordem total dos simplexos
\begin{align*}
    \sigma^1 < \sigma^2 < \cdots < \sigma^n
\end{align*}
tal que 
\begin{align*}
    \forall \sigma, \tau \in K, \,\, \tau \nsubseteq \sigma \implies \tau < \sigma.
\end{align*}
Em outras palavras, uma face de um simplexo é inferior ao próprio simplexo. Para todo $i \leq n$,
considere o complexo simplicial
\begin{align*}
    K^i = \{\sigma^1, \cdots, \sigma^i \}.
\end{align*}
Temos a relação $K^{i+1} = K^i \cup \{\sigma^{i+1}\}$. Eles formam uma sequência crescente de
complexos simpliciais
\begin{align*}
    K^1 \subset K^2 \subset \cdots \subset K^n,
\end{align*}
com $K^n = K$.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K1.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K2.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K3.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K4.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K5.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K6.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K7.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K8.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K9.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.18\textwidth}
        \includegraphics[width=\textwidth]{Imagens/evolucao_complexo_K10.png}
    \end{subfigure}
    \hfill
\end{figure}
\ident Usemos a representação acima para construir uma cadeia encaixante. Seja $X \subset \mathbb{R}^n$. A coleção de seus espessamentos é uma sequência não decrescente de subconjuntos
\[
...
\end{align*}
onde as aplicaçôes $(i_s^t)_*$ são aquelas induzidas na homologia pelas inclusões $i_s^t$. Nessa sequência,podemos observar por quanto tempo um ciclo vive, ou persiste. Sejam $i \geq 0$, $t_0 \geq 0$ e considere um
...
\begin{definition}
    ...
    \]
    O termo \enquote{multiconjunto} significa que $\mathcal{I}$ pode conter várias cópias do mesmo intervalo $I$. Diz-se que tal módulo é decomponível em módulos intervalares, ou simplesmente decomponível.
\end{definition}

\begin{theorem}
    Se um módulo de persistência $\mathbb{V}$ se decompõe em módulos intervalares, então o multiconjunto $\mathcal{I}$ de intervalos é único. Além disso, se todos espaços vetoriais $V^t$ de $\mathbb{V}$ tiverem dimensão finita, então $V$ se decompõe em módulos intervalares.
\end{theorem}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Imagens/simplexo_diagrama.png}
        \caption{Complexo Simplicial}
    \end{subfigure}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Construção da paisagem de persistência do Complexo (a). 
    (b)–(c) Barcode e diagrama de persistência padrão para $H_1$. (d) O mesmo diagrama com os triângulos de cada ponto $(b,d)$ explicitados: a perna vertical (nascimento) e a horizontal (morte) ligam o ponto à diagonal. (e) Cada triângulo de (d) corresponde a uma \enquote{tenda} $f_i(t)=\max(0,\min(t-b_i,\,d_i-t))$; a landscape $\lambda_k(t)$ é o $k$-ésimo maior valor dessas tendas em cada $t$, plotada aqui camada a camada. (f) As mesmas camadas $\lambda_k$, agora dispostas como um relevo tridimensional $(t, k, \lambda_k(t))$, evidenciando a estrutura de múltiplas camadas da landscape do complexo.}
    \label{fig:pipeline_landscape}
\end{figure}

\begin{definition}
Seja $(\Omega, \mathcal{A}, \mu)$ um espaço de medida. Para $1 \le p < \infty$, o espaço de Lebesgue $L^p(\Omega)$ é definido como a coleção de todas as funções mensuráveis $f: \Omega \to \mathbb{R}$ cujas potências $p$-ésimas são integráveis segundo Lebesgue:
\begin{equation*}
L^p(\Omega) = \left\{ f: \Omega \to \mathbb{R} \;\middle|\; \|f\|_p = \left( \int_{\Omega} |f(x)|^p \, d\mu(x) \right)^{1/p} < \infty \right\}.
\end{equation*}
Para o caso limite $p = \infty$, o espaço $L^\infty(\Omega)$ consiste em funções essencialmente limitadas munidas da norma $\|f\|_\infty = \text{ess\,sup}_{x \in \Omega} |f(x)|$.
\end{definition}

\begin{definition}
Um espaço vetorial normado $(V, \|\cdot\|)$ é denominado Espaço de Banach se ele é completo com respeito à métrica induzida por sua norma, isto é, se toda sequência de Cauchy em $V$ converge para um elemento pertencente a $V$.
\end{definition}

\begin{definition}
Um Espaço de Hilbert $\mathcal{H}$ é um espaço vetorial dotado de um produto interno $\langle \cdot, \cdot \rangle_{\mathcal{H}}$ que é completo com respeito à norma induzida $\|f\|_{\mathcal{H}} = \sqrt{\langle f, f \rangle_{\mathcal{H}}}$.
\end{definition}

Ao fixar $p = 2$, o espaço de Lebesgue $L^2(\mathbb{N} \times \mathbb{R})$ torna-se um Espaço de Hilbert, em que o produto interno entre duas paisagens de persistência $\lambda_A$ e $\lambda_B$ é estipulado por:
\begin{equation*}
\langle \lambda_A, \lambda_B \rangle_{L^2} = \sum_{k=1}^{\infty} \int_{\mathbb{R}} \lambda_A(k, t) \, \lambda_B(k, t) \, dt.
\end{equation*}

A existência do produto interno em $L^2$ aciona diretamente o Teorema de Representação de Riesz:

\begin{theorem}
Seja $\mathcal{H}$ um Espaço de Hilbert e seja $\mathcal{H}^*$ seu espaço dual topológico. Para todo funcional linear contínuo $\phi \in \mathcal{H}^*$, existe um único elemento $y_{\phi} \in \mathcal{H}$ tal que:
\begin{equation*}
\phi(x) = \langle x, y_{\phi} \rangle_{\mathcal{H}}, \quad \forall x \in \mathcal{H}.
\end{equation*}
Além disso, a norma do funcional coincide com a norma do vetor representante: $\|\phi\|_{\mathcal{H}^*} = \|y_{\phi}\|_{\mathcal{H}}$.
\end{theorem}
...
\ident O corolário analítico mais profundo advindo da formalização das Paisagens de Persistência reside no fato de que cada função $\lambda_k(t)$ constitui, por construção, um elemento do espaço integrável de Lebesgue $L^p(\mathbb{N} \times \mathbb{R})$. Dado que o espaço $L^p$ é dotado de uma estrutura de Espaço de Banach — caracterizando-se como um espaço vetorial normado e completo —, as limitações algébricas e métricas inerentes aos diagramas de persistência não lineares são superadas. Em particular, ao fixar $p = 2$, a emergência de um Espaço de Hilbert possibilita a definição rigorosa do produto interno $\langle \lambda, \lambda' \rangle_{L^2}$, assegurando pela aplicabilidade do Teorema de Representação de Riesz que qualquer funcional linear contínuo sobre este espaço funcional admita uma representação integral única. Essa transição da combinatória discreta dos complexos simpliciais para a continuidade analítica em espaços de Hilbert fundamenta a evocação do Teorema de Mercer, o qual garante a existência de um \textit{kernel} simétrico e definido positivo associado a esse produto interno. É precisamente essa imersão funcional em um espaço vetorial linearmente estruturado que pavimenta o caminho para a modelagem supervisionada via Máquinas de Vetores de Suporte (que será formalmente desenvolvida na Subseção \ref{secao_svm}), provendo o suporte matemático para o cálculo do hiperplano ótimo de separação entre os diferentes estados dinâmicos da proteína.
...
\begin{theorem}[Teorema dos Grandes Números]
    Seja $X_1, X_2, \cdots, X_n$ uma sequência de variáveis aleatórias independentes e identicamente distribuídas (i.i.d.), com média (valor esperado) finito $E(X_i) = \mu$. A média amostral é definida por: 
    \[
        \overline{X} = \frac{1}{n} \sum_{i=1}^{n}X_i.
    \]
\end{theorem}
O teorema garante que, sob o limite de infinitas amostras ($n \rightarrow \infty $), a média amostral converge para o valor esperado real da população:
\[
\overline{X}_n\xrightarrow{n \rightarrow \infty} \mu.
\]

\begin{theorem}[Teorema Central do Limite]
    Sejam $X_1, X_2, \cdots, X_n$  os elementos de uma amostra aleatória extraída de uma distribuição com média $\mu$ e e variância $\sigma^2$ finitas. Para quaisquer $n$:
    \begin{enumerate}
        \item O valor esperado da média amostral $\overline{X}$ é igual à média populacional: $E(\overline{X} = \mu)$
        \item A variância da média amostral é reduzida pelo tamanho da amostra: $V(\overline{X})= \frac{\sigma^2}{n}$.
    \end{enumerate}
    À medida que $n$ cresce para o infinito ($n \rightarrow \infty$), a média amostral $\overline{X}$ converge em distribuição para uma normal:
    \begin{align*}
        \overline{X} \xrightarrow{d} N\left(\mu, \frac{\sigma^2}{n}\right).
    \end{align*}
    Dessa forma, a variável padronizada $Z$ se aproxima da distribuição normal padrão $N(0, 1)$:
    \begin{align*}
        Z = \frac{\overline{X}-\mu}{\sigma/\sqrt{n}} \xrightarrow{d} N(0,1).
    \end{align*}
\end{theorem}

\ident Devido à estrutura do espaço de Banach, a variável $X$ satisfaz a Lei dos Grandes Números e o Teorema Central do Limite \cite{Bubenik2015} \cite{devore2012}. Isso implica que, para uma amostra suficientemente grande ($n \to \infty$), a distribuição da média amostral $\overline{X}$ aproximar-se-ia de uma distribuição Normal. Contudo, em contextos biológicos e cristalográficos, o número de estruturas resolvidas com alta qualidade é frequentemente limitado. 

\subsection{O Teste de Permutação Não-Paramétrico}

\ident No estudo de Kovacev-Nikolic, a amostra disponível continha apenas $n = 14$ estruturas de MBP (sendo sete na forma aberta e sete na fechada) \cite{kovacev2016}. Com amostras tão diminutas, a suposição de normalidade exigida por testes paramétricos clássicos não pode ser garantida com rigor \cite{devore2012}.
...
\ident A fundamentação deste teste serve como o selo de validação estatística da nossa modelagem. No entanto, a análise não se esgota na inferência estática. Ao comprovar que as distribuições são separáveis, o próximo passo lógico é extrair as variáveis topológicas vetorizadas e introduzi-las num ambiente de Aprendizagem de Máquina (\textit{Machine Learning}), com o intuito de construir fronteiras de decisão preditivas para classificar novas estruturas moleculares.
...
\subsection{Visualização Não-Linear: ISOMAP e Gráfico Scree} \label{sec:isomap}
...
\ident O sucesso algébrico do algoritmo depende da escolha correta do parâmetro de dimensão embutida $d$. Para determinar o valor ideal, ou seja, a dimensionalidade intrínseca que captura a maior parte da variância da proteína sem introduzir ruído, recorre-se ao Gráfico Scree (\textit{Scree plot}). Este gráfico plota o erro residual da projeção em função do número de dimensões. O ponto de inflexão da curva (frequentemente chamado de \enquote{cotovelo}) indica a dimensão ótima. Estudos anteriores, como o de Kovacev \cite{kovacev2016}, demonstram que, para os dados da MBP, a projeção tridimensional ($d=3$) atinge a estabilidade do erro, sendo considerada apropriada para a separação geométrica dos estados conformacionais \cite{kovacev2016}.
...
\ident É importante notar que o ISOMAP, por depender de construções de um grafo de vizinhança e distâncias geodésicas sobre a amostra completa, não admite uma extensão paramétrica trivial para projetar pontos fora da amostra original (\emph{out-of-sample}). Por esse motivo, seu uso neste trabalho restringe-se ao diagnóstico visual de dimensionalidade intríseca da matriz de distâncias dinâmicas $D$, não sendo empregado na etapa de classificação supervisionada, onde a exigência de uma projeção explícita e livre de vazamento de dados sobre validação torna a escolha metodologicamente correta.
...
\begin{equation}\label{eq: hiperplano}
    f(x) = w \cdot x + b = 0
\end{equation}
em que $w \cdot x$ é o produto escalar entre os vetores $w$ e $x, w \in X$ é o vetor normal ao hiperplano descrito e $\frac{b}{\lVert w\rVert}$ corresponde à distância do hiperplano em relação à origem, com $b \in \mathbb{R}$.

\ident O hiperplano divide o espaço dos dados $X$ em duas regiiões: $w \cdot x + b > 0$ e $w \cdot x + b < 0$.  Podemos definir a função sinal
\begin{equation*}
...
\ident Tem-se que $w \cdot x_1 + b = +1$ e $w \cdot x_2 + b = -1$. A diferença entre essas equações fornece $w \cdot (x_1 - x_2) = 2$, logo, obtem-se:
\begin{equation*}
...
\end{align*}
\ident As restrições são impostas de maneira a assegurar que não haja dados de treinamento
...
\ident O problema de otimização obtido é quadrático, cuja solução possui uma ampla e estabelecida teoria matemática \cite{lorena2007}. Como a função objetivo sendo minimizada é convexa e os pontos que satisfazem as restrições formam um conjunto convexo, esse problema possui um único mínimo global. Problemas desse tipo podem ser solucionados com a introdução de uma função Lagrangiana, que engloba as restrições à função objetivo, associadas a parâmetros denominados multiplicadores de Lagrange $\alpha_i$.
\begin{align*}
    L(w,b,\alpha) = \frac{1}{2} \lvert w\rvert^2 - \sum_{i=1}^{n} \alpha_i(y_i(w \cdot x_i + b)-1).
\end{align*}
\ident A função Lagrangiana deve ser minimizada, o que implica em maximizar as variáveis $\alpha_i$ e minimizar $w$ e $b$. Tem-se então um ponto de sela, no qual:
\begin{align*}
    \frac{\partial L}{\partial b} = 0 \quad \text{e} \quad \frac{\partial L}{\partial w} = 0.
\end{align*}
A resolução dessas equações leva aos resultados abaixo:
\begin{align*}
...
\ident Obtida a solução ótima do problema dual, representada pelo vetor de multiplicadores de Lagrange $\alpha^*$, as variáveis primais podem ser restabelecidas. O vetor de pesos $w^*$ é determinado por uma combinação linear ponderada dos dados de treinamento, sofrendo a influência direta apenas das observações cujos multiplicadores associados sejam estritamente positivos ($\alpha_i^* > 0$).

\ident Para que uma solução de um problema de programação não linear seja ótima, devemos definir as seguintes condições, chamadas de Condições de Karush-Kuhn-Tucker (KKT):

\begin{enumerate}
    \item \textbf{Estacionariedade (Anulação dos Gradientes):}
    \begin{align*}
    ...
    \frac{\partial \mathcal{L}}{\partial \xi_i} = C - \alpha_i - \mu_i = 0 & \implies 0 \le \alpha_i^* \le C.
    \end{align*}
    
    \item \textbf{Viabilidade Primal:}
    \begin{equation*}
    y_i (w^* \cdot x_i + b^*) - 1 + \xi_i^* \ge 0 \quad \text{e} \quad \xi_i^* \ge 0, \quad \forall i.
    \end{equation*}
    
    \item \textbf{Viabilidade Dual:}
    \begin{equation*}
    \alpha_i^* \ge 0 \quad \text{e} \quad \mu_i^* \ge 0, \quad \forall i.
    \end{equation*}
...
\ident Como resultado final deste processo de otimização, estabelece-se uma lei de decisão descontínua $g(x)$ e não linear definida pela aplicação do operador sinal ($\text{sgn}$) sobre o funcional afim $f(x)$:
\begin{equation*}
    g(x) = \text{sgn}\left( \sum_{i \in \mathcal{S}} \alpha_i^* y_i x_i\cdot x + b^* \right).
\end{equation*}
O hiperplano atua como a fronteira de transição contínua onde $f(x) = 0$, garantindo a separação das classes com margem máxima e ótima capacidade de generalização estatística.
...
\ident É fundamental destacar que a própria construção das Paisagens de Persistência resolve a não-linearidade geométrica intrínseca às flutuações proteicas. Como demonstrado na Seção \ref{sec:paisagens_persitencia}, ao adotar o espaço integrável de Lebesgue com $p=2$, o domínio das paisagens assume a estrutura de um espaço de Hilbert funcional munido do produto interno $\langle\lambda,\lambda'\rangle_{L^2}$. Dessa forma, o pré-processamento topológico mapeia a complexidade combinatória e tridimensional da proteína em representações funcionais linearmente bem-comportadas. Consequentemente, a distinção entre as conformações \textit{Apo} e \textit{Holo} pode ser operada diretamente por uma Máquina de Vetores de Suporte Linear — empregando a formulação de margem suave para acomodar eventuais sobreposições e ruídos no espaço de atributos —, onde o hiperplano separador opera diretamente sobre o produto escalar vetorial. A regra de decisão final é expressa por:
\[
    g(x) = \text{sgn}\left( w^* \cdot x + b^* \right) = \text{sgn}\left( \sum_{i \in \mathcal{S}} \alpha_i^* y_i (x_i \cdot x) + b^* \right),
\]
onde $\mathcal{S}$ representa o conjunto de índices associados aos vetores de suporte calculados no problema dual.
...
euclidiana da diferença entre as conformações \textit{Apo} e \textit{Holo} da MBP, e estabelecendo
...
\begin{algorithmic}[1]
\Require Lista de identificadores $\mathcal{L} = \{id_1, id_2, \dots, id_M\}$, Categoria da proteína $c$ (\textit{Apo/Holo}).
\Ensure Salvamento das matrizes de distância.
...
\For{cada identificador $id \in \mathcal{L}$} % Reabertura artificial para manter as linhas verticais/indentação
    ...
    \State $\text{SalvarMatriz}(D)$
\EndFor
\end{algorithmic}
\end{algorithm}

\ident A transformação das flutuações tridimensionais nas matrizes de distância funcional de dimensão $370 \times 370$ converteu o sistema biofísico em um espaço de dissimilaridade discreto. Esta representação matricial constitui o dado de entrada exato sobre o qual serão aplicadas as filtrações simpliciais para a extração dos invariantes topológicos.

\subsection{Filtração de Vietoris-Rips e Paisagens de Persistência}

\ident Para extrair os invariantes topológicos das matrizes de dissimilaridade dinâmica $D$, computou-se o complexo simplicial de Vietoris-Rips. O limiar máximo de conexão foi fixado analiticamente em $t_{\max} = 0.7$. Esta escolha fundamenta-se estritamente no ciclo de vida dinâmico das características homológicas da proteína de ligação à maltose (MBP): observações empíricas e a literatura clássica \cite{kovacev2016} revelam que os laços unidimensionais mais pronunciados no grupo de homologia de grau 1 ($H_1$) — associados diretamente às flutuações mecânicas da fenda de ligação — nascem em torno de $t \approx 0.2$ e sofrem morte topológica pouco antes de $t = 0.6$. A projeção visual das assinaturas de persistência confirma o esgotamento de novos picos persistentes após esse limiar. Portanto, o truncamento em $t_{\max} = 0.7$ garante o mapeamento analítico de toda a trajetória vital dos geradores homológicos relevantes, ao mesmo tempo em que otimiza a eficiência algorítmica ao evitar o cálculo de conexões triviais e ruídos combinatórios em escalas superiores. A progressão geométrica desta filtração computacional está ilustrada na Figura \ref{fig:evolucao_rips}.
...
\ident Para contornar a não-linearidade métrica inerente aos diagramas de persistência brutos e viabilizar a aplicação direta de classificadores estatísticos, cada diagrama foi mapeado em sua respectiva Paisagem de Persistência \cite{Bubenik2015}. A vetorização contínua dessas estruturas funcionais empregou parâmetros calibrados analiticamente para o grau $g=1$: o domínio de filtração foi delimitado no intervalo $t \in [0.0, 0.7]$ com as características de persistência infinita truncadas no valor máximo do limiar, sob uma discretização rigorosa de $50$ pontos equidistantes ($t_j$). A determinação do número de contornos funcionais ordinais foi fixada em exatamente $K = 73$ camadas camadas transversais ($\lambda_1, \lambda_2, \dots, \lambda_{73}$), quantidade exata estabelecida pela partição matemática dos módulos de persistência para o grau 1 da MBP \cite{kovacev2016}, sendo suficiente para encapsular a totalidade das variações geométricas estáveis do sistema. A Figura \ref{fig:comparativo_landscapes} contrasta o perfil topológico contínuo resultante para ambas as classes estruturais.
...
\ident O fluxo do mapeamento topológico está delineado no Algoritmo \ref{alg:landscapes}. Ao amostrar 50 pontos contínuos para cada uma das 73 curvas escalonadas da paisagem, o procedimento gera uma matriz funcional que, ao ser aplanada sequencialmente, resulta em um vetor descritor fixo de exatamente $3650$ dimensões ($73 \times 50$). Este procedimento finaliza a engenharia de dados, imergindo as flutuações conformacionais da proteína em um subespaço de Banach idealizado para modelos lineares de aprendizado de máquina.
...
\ident Ressalva-se que a redução de dimensionalidade aqui aplicada ao vetor de paisagens via PCA é independente da análise ISOMAP da Seção \ref{sec:isomap}, a qual operou unicamente sobre a matriz de distâncias dinâmicas $D$ com finalidade exploratória estrutural.

\ident Com as flutuações biomoleculares rigorosamente mapeadas num espaço de Hilbert funcional, o problema de distinguir os estados conformacionais resume-se ao reconhecimento de padrões em alta dimensionalidade. Conforme detalhado na seção anterior, a discretização dos $k=73$ contornos funcionais ordinais sobre 50 pontos equidistantes gerou, após o aplanamento da matriz de características, um espaço vetorial composto por 3650 dimensões para cada estrutura proteica. Face ao tamanho restrito do conjunto de calibração disponível ($n=14$), o modelo encontra-se sob um regime de severa superparametrização ($p \gg n$), suscetível ao sobreajuste (\textit{overfitting}) e aos efeitos degenerativos da maldição da dimensionalidade. Para mitigar esse fenômeno, aplicou-se a Análise de Componentes Principais (PCA) sobre a matriz de dados previamente padronizada, truncando o espaço ortogonal nas 3 primeiras componentes principais. Essa escolha metodológica baseia-se no comportamento espectral do sistema tridimensional: no grupo homológico $H_1$, estas três componentes concentram exatamente $52,50\%$ da variância total explicada das paisagens topológicas \cite{kovacev2016}, retendo a informação geométrica estruturante da flexibilidade proteica enquanto descartam variações espúrias.
...
\ident O núcleo do aprendizado supervisionado foi operado por uma Máquina de Vetores de Suporte (SVM) parametrizada com um \textit{kernel} linear e constante de regularização $C=1.0$. Salienta-se que, embora a formulação analítica das paisagens de persistência garanta nativamente as propriedades de um Espaço de Hilbert separável $L^2(\mathbb{N} \times \mathbb{R})$ permitindo a aplicação conceitual do Teorema de Mercer via representações intrínsecas de produto interno em dimensão infinita, a etapa algorítmica requer uma aproximação finita estável. Desta forma, a discretização em vetores de $\mathbb{R}^{3650}$ seguida pela projeção linear em um subespaço truncado de três componentes principais atua como um regularizador metodológico fundamental. Essa arquitetura simplificada reduz o risco de sobreajuste (\textit{overfitting}) em regimes onde o número de atributos supera severamente o tamanho amostral ($p \gg n$), permitindo que o hiperplano de margem máxima opere com robustez estatística sobre as assinaturas de conectividade topológica.
...
\begin{algorithmic}[1]
\Require Matriz de características $X \in \mathbb{R}^{N \times 3650}$, Vetor de rótulos $Y \in \{-1, +1\}^N$, Conjunto de teste $X_{\text{novas}}$.
...
\ident Concluído este ensaio de validação e obtendo-se o modelo analítico final embasado nos dados de calibração, o escopo metodológico da pesquisa considera-se encerrado. O fluxo computacional estabelecido assegura que todas as inferências sobre as novas dezenas de proteínas coletadas apoiem-se sobre métricas invariantes a translações rígidas e validadas estatisticamente. No capítulo subsequente, apresentam-se e discutem-se os resultados numéricos, as visualizações das projeções não-lineares e a eficácia de classificação decorrente desta arquitetura.

\subsection{Modelagem Preditiva e Classificação Geométrica}
...
\ident Como contraponto analítico ao método topológico (\textit{baseline} de validação), foi implementado a classificação puramente geométrica baseada no Desvio Quadrático Médio (RMSD). Empregando o alinhamento de Kabsch sobre os carbonos-alfa idênticos, o conjunto de testes $S$ foram projetadas contra os dois conjuntos de calibração: $\mathcal{A}$ (7 estruturas Apo) e $\mathcal{H}$ (7 estruturas Holo). Para garantir robustez e afastar o viés de uma escolha arbitrária de uma única proteína referencial, a classificação geométrica foi bifurcada em duas abordagens exaustivas:
...
\If{$\text{VotosHolo} > (\text{TotalCombinações} / 2)$}
    \State \Return \enquote{Holo}, $\text{Certeza}$
\Else
    \State \Return \enquote{Apo}, $(100 - \text{Certeza})$
\EndIf
\end{algorithmic}
\end{algorithm}
...
\ident Aplicando o Teste de Permutação à variável $X$ sobre as 14 estruturas de calibração, obteve-se $\overline{X}_{\text{Holo}} = 0{,}269778$ e $\overline{X}_{\text{Apo}} = 0{,}290403$, resultando em $T_{\text{obs}} = |\overline{X}_1 - \overline{X}_2| = 0{,}020625$. Das $M = \binom{14}{7} = 3432$ permutações exaustivas dos rótulos, apenas $2$ produziram uma diferença de médias igual ou superior a $T_{\text{obs}}$, resultando no valor-$p$ empírico $p = 2/3432 \approx 5{,}83 \times 10^{-4}$. Como $p \ll \alpha = 0{,}05$, rejeita-se $H_0$ com margem substancial, confirmando estatisticamente que a área total sob as paisagens de persistência e, por extensão, a assinatura topológica global das flutuações dinâmicas difere de forma não atribuível ao acaso entre as conformações Apo e Holo. É importante notar que este teste avalia a média de uma estatística escalar agregada, não a separabilidade linear do vetor completo de 3650 dimensões e constituindo, portanto, evidência estatística independente e prévia à etapa de classificação supervisionada, e não uma prova da separabilidade em si.
...
\ident A Tabela \ref{tab:matriz_confusao}  sumariza o desempenho quantitativo dos dois classificadores no conjunto de 29 estruturas de validação independente.
...
\begin{table}[h]
    ...
    \begin{tabular}{lccccc}
        \hline
        Modelo & Acurácia & Precisão (Holo) & Sensibilidade (Holo) & Especificidade (Apo) & F1 (Holo) \\
        \hline
        SVM Topológico & 27/29 (93.10\%) & 91.67\% & 91.67\% & 94.12\% & 91.67\% \\
        RMSD Médio (\emph{baseline}) & 26/29 (89.66\%) & 84.62\% & 91.67\% & 88.24\% & 88.00\% \\
        \hline
    \end{tabular}}
    \caption{Comparação quantitativa entre o classificador topológico (SVM sobre paisagens de persistência) e o \emph{baseline} geométrico (RMSD médio), sobre as 29 estruturas mineradas independentemente do conjunto de calibração.} \label{tab:matriz_confusao}
\end{table}

\ident O SVM topológico superou o RMSD Médio em 3.4 pontos percentuais de acurácia global, cometendo o mesmo número de falsos negativos (1, em ambos os casos referente à estrutura 1DMB), porém menos falsos positivos (1 contra 2). Note-se que, do ponto de vista estritamente numérico, a diferença de dois pontos de erro num total de 29 amostras (um único caso: \texttt{2MV0} para o RMSD, e \texttt{2N45} para o SVM, são os únicos erros que distinguem os dois métodos) não permite, por si só, uma afirmação de superioridade estatisticamente robusta sem um teste formal (e.g. teste de McNemar sobre amostras pareadas); a discussão que segue enfatiza, portanto, os casos qualitativamente distintos como evidência mais informativa do que a diferença bruta de acurácia.

\ident A análise global da Tabela \ref{tab:comparativo_geral} revela uma consistência entre os métodos geométrico e topológico para a vasta maioria do conjunto de dados, atestando a validade da abstração por Paisagens de Persistência. Contudo, a identificação empírica de falhas no alinhamento métrico e de fronteiras de decisão topológicas forneceu achados matemáticos e biofísicos cruciais, delineados a seguir.
...
\ident Cabe ressaltar que o algoritmo de alinhamento estrutural que baseia o cálculo clássico do RMSD requer correspondência estrita de índices entre as matrizes de coordenadas. Consequentemente, o método geométrico colapsa diante de falhas de indexação do PDB ou \emph{gaps} na cadeia peptídica (ausência de truncamento prévio). O comportamento anômalo registrado na estrutura \texttt{7Z7H} não representa um desvio biológico, mas demonstra precisamente a maior vantagem prática da Inferência Topológica: por operar unicamente sobre a matriz intrínseca de conectividade do Modelo de Rede Anisotrópica (ANM), a topologia algébrica é naturalmente invariante a artefatos de numeração linear e ruídos de alinhamento, isolando o sinal biológico da limitação geométrica posicional.
...
\ident Essa discordância isolada não representa uma falha algorítmica, mas a detecção de uma fase de transição contínua. Por utilizar distâncias dinâmicas provenientes das flutuações de B-factor, o método topológico rastreia flexibilidade latente. Este resultado limítrofe é fortemente compatível com a hipótese de seleção conformacional descrita na literatura, sugerindo que o método topológico é capaz de capturar estados transientes.
...
\section{Topologia} \label{apx:topologia}
...
\section{Estabilidade Estatística} \label{apx:loocv}

\ident A validação cruzada \textit{Leave-One-Out} (LOOCV) fornece um estimador não viesado do erro de generalização. O Teorema de Luntz e Brailovsky estabelece um limite superior analítico para a esperança do erro do LOOCV baseado unicamente na cardinalidade esperada do conjunto de vetores de suporte.

\begin{theorem}
Seja $D_N = \{(x_1, y_1), \dots, (x_N, y_N)\}$ um conjunto de dados de treinamento sorteado de forma independente e identicamente distribuída (i.i.d.). Considere o classificador SVM treinado sobre subconjuntos de tamanho $N-1$. A esperança do erro do estimador \textit{Leave-One-Out}, $\mathbb{E}[R_{\text{LOO}}(N)]$, satisfaz rigorosamente:
\begin{equation*}
\mathbb{E}\left[ R_{\text{LOO}}(N) \right] = \frac{\mathbb{E}\left[ |\mathcal{S}_{N-1}| \right]}{N},
\end{equation*}
em que $|\mathcal{S}_{N-1}|$ representa o número de vetores de suporte produzidos pelo modelo ao ser treinado em um conjunto reduzido de $N-1$ amostras.
\end{theorem}

\begin{proof}
Seja $g_{D \setminus \{i\}}$ o classificador obtido ao remover a $i$-ésima amostra do conjunto $D_N$. O erro do LOOCV é expresso por 
\[
R_{\text{LOO}}(N) = \frac{1}{N} \sum_{i=1}^{N} \mathbb{I}\left( g_{D \setminus \{i\}}(x_i) \neq y_i \right)
\]
em que $\mathbb{I}$ é a função indicadora. 

Se uma amostra $(x_i, y_i)$ reanalisada não for um vetor de suporte no treinamento completo, ela estaria estritamente no interior da margem correta; logo, a sua remoção não altera o hiperplano de separação, implicando $g_{D \setminus \{i\}}(x_i) = y_i$ e erro nulo. Um erro de classificação em $x_i$ só pode ocorrer se $x_i \in \mathcal{S}_N$. Tomando a esperança matemática em ambos os lados e aplicando a simetria da distribuição i.i.d., obtém-se o limite proposto.
\end{proof}
