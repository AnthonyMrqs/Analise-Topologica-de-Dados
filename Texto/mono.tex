...
Para capturar exclusivamente a mecânica cooperativa global --- isto é, o movimento de dobradiça essencial para a captura do ligante --- opera-se uma restrição dimensional. Retêm-se apenas os primeiros $M = 20$ modos não-triviais de menor autovalor (do índice $k=7$ ao $k=6+M$). A matriz pseudoinversa de Moore-Penrose truncada, $\mathbf{H}^+_{\text{trunc}}$, é reconstruída estritamente sobre este subespaço:

\begin{equation}\label{eq:pseudoinversa_truncada}
\mathbf{H}^+_{\text{trunc}} = \sum_{k=7}^{6+M} \frac{1}{\lambda_k} \mathbf{u}_k \mathbf{u}_k^T.
\end{equation}
...
Esta conversão mapeia o domínio de $[-1, 1]$ da correlação para um espaço pseudométrico rigoroso $[0, 1]$, carregando implicações estatísticas e topológicas profundas. É crucial notar que $\mathbf{D}$ não é estritamente uma métrica, mas sim uma \textbf{pseudométrica}. Isto ocorre porque a identidade dos indiscerníveis não se verifica rigorosamente: dois resíduos distintos ($i \neq j$) podem oscilar de forma perfeitamente correlacionada ($|C_{ij}| = 1$), resultando numa distância $D_{ij} = 0$. A Topologia Algébrica, contudo, é perfeitamente bem definida sobre espaços pseudométricos, permitindo a construção de complexos simpliciais sem perda de generalidade teórica.
...
\begin{remark}[Nota sobre o Fluxo Metodológico]
Ressalta-se que a aplicação do ISOMAP possui um propósito estritamente exploratório e visual no contexto deste estudo, permitindo a inspeção gráfica da dimensionalidade intrínseca (Gráfico Scree). O fluxo computacional analítico de classificação (\textit{Machine Learning}) \textbf{não} transita por este subespaço tridimensional gerado pelo ISOMAP. A extração de características preditivas opera diretamente e independentemente sobre as paisagens de persistência através de Análise de Componentes Principais (PCA), conforme detalhado na secção do pipeline computacional.
\end{remark}
...
No contexto deste estudo, substituímos o espaço euclidiano $\mathbb{R}^3$ pelo espaço probabilístico das interações dinâmicas da MBP. Assim, o complexo $\mathrm{Rips}^t(X)$ instanciará um $1$-simplexo (aresta) entre dois resíduos se a distância dinâmica $D_{ij} \leq 2t$. Conforme demonstrado por Cang et al.~\cite{cang2015}, esta substituição dota os invariantes de uma ontologia puramente mecanicista:
\begin{enumerate}
...
% -------------------------------------------------------------------
\subsection{O Pipeline Computacional do Estudo}
\label{sec:pipeline}
% -------------------------------------------------------------------

Para garantir a reprodutibilidade e a clareza da transição entre a fundamentação teórica e a experimentação empírica, o fluxo de processamento analítico foi estruturado nas seguintes etapas sequenciais:

\begin{enumerate}
    \item \textbf{Dinâmica e Pseudométrica:} A partir das coordenadas cristalográficas ($C_\alpha$), constrói-se o Modelo de Rede Anisotrópica (ANM) e deriva-se a matriz de correlação cruzada $\mathbf{C}$, mapeada na pseudométrica dinâmica $\mathbf{D}$.
    \item \textbf{Homologia Persistente:} Aplica-se a filtração de Vietoris-Rips sobre $\mathbf{D}$, extraindo-se os diagramas de persistência referentes ao grupo de homologia $H_1$ (ciclos de comunicação alostérica).
    \item \textbf{Vetorização via Paisagens de Persistência:} Os diagramas são mapeados no espaço funcional de Banach e convertidos em vetores. Empiricamente, o número de características topológicas detetadas ditou o hiperparâmetro estrutural de $k_{\max} = 73$ funções tenda (camadas). Discretizando cada camada em 50 pontos, cada proteína é representada por um vetor em $\mathbb{R}^{3650}$.
    \item \textbf{Redução de Dimensionalidade Analítica (PCA):} O espaço $\mathbb{R}^{3650}$ é reduzido linearmente através de Análise de Componentes Principais (PCA). Os primeiros 3 componentes, retendo a maior variância do sistema, formam o novo espaço de características.
    \item \textbf{Classificação (LOO-SVM):} Submete-se o espaço reduzido a um classificador de Máquinas de Vetores de Suporte (SVM) linear, validado estritamente por validação cruzada \textit{Leave-One-Out} devido ao tamanho da amostra ($n=14$).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{O limite prático do RMSD rígido (7Z7H):} A estrutura \texttt{7Z7H} acusou um RMSD divergente na ordem de $\sim 26$ \AA. Em biologia estrutural, um desvio desta magnitude não traduz uma mera mudança conformacional funcional, sugerindo antes problemas severos no alinhamento tridimensional (tais como ausência de cadeias homólogas, numeração de resíduos não padrão ou artefatos de resolução espacial profunda). Perante anomalias que inviabilizam o alinhamento de corpo rígido, o algoritmo de Kabsch perde a sua interpretabilidade física. O facto de o modelo SVM ter processado a proteína normalmente, gerando um vetor de distâncias dinâmicas válido baseado na conetividade interna relativa, ilustra uma vantagem mecânica da Topologia: por basear-se em distâncias intra-moleculares (matriz $\mathbf{D}$), a TDA mostra-se resiliente a distorções coordenadas extremas que corrompem fatalmente os descritores euclidianos absolutos.
\end{itemize}
